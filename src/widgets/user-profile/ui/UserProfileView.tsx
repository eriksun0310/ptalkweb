...
import { getAvatarSource } from '@/shared/constants/icons';
...
export const UserProfileView: React.FC<UserProfileViewProps> = ({ user, comments, totalCount }) => {
    ...
    <div className="min-h-screen bg-white pb-8">
      <div className="max-w-2xl mx-auto px-4 pt-20">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          <UserAvatar src={avatarSrc} alt={user.name} size="xl" className="mb-3" />
          <h1 className="text-xl font-bold text-gray-900 mb-1">{user.name}</h1>
          <p className="text-sm text-gray-600">{totalCount} 則評論</p>
        </div>
            ...
            {totalCount > comments.length && (
              <div className="mt-6 p-6 bg-gray-100 rounded-lg text-center">
                <p className="text-sm text-gray-900 font-semibold mb-2">
                  還有 {totalCount - comments.length} 則評論
                </p>
